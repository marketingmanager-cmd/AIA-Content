...
# OPENAI จำเป็น (ใช้สร้างคอนเทนต์) — Telegram ไม่บังคับ (จะใช้แค่ Facebook ก็ได้)
if not OPENAI_API_KEY:
    print("❌ ไม่พบ OPENAI_API_KEY ใน .env")
    raise SystemExit
if not (TELEGRAM_TOKEN and TELEGRAM_CHAT_ID):
    print("⚠️ ไม่มี TELEGRAM_TOKEN/CHAT_ID — ปิดการโพสต์ผ่าน Telegram (ใช้ Facebook ได้ปกติ)")

API = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}"
client = OpenAI(api_key=OPENAI_API_KEY)

# ---------- ตั้งค่าแบรนด์: ธุรกิจ + ลูกค้า + โทน (แก้ตรงนี้ให้ตรงกับตัวคุณ) ----------
BRAND = "ตัวแทนประกันชีวิตและประกันสุขภาพ AIA — ให้คำปรึกษาวางแผนความคุ้มครองชีวิต สุขภาพ โรคร้ายแรง อุบัติเหตุ การออม/บำนาญ และวางแผนการเงินสำหรับบุคคลและครอบครัว"
AUDIENCE = "คนทำงาน เจ้าของธุรกิจ พ่อแม่/หัวหน้าครอบครัว และคนที่อยากวางแผนคุ้มครองสุขภาพ-ชีวิต-การเงิน รวมถึงคนที่กำลังมองหาประกันสุขภาพ/ประกันชีวิต/ประกันโรคร้ายแรง/วางแผนเกษียณ"
TONE = "อบอุ่น ใส่ใจ น่าเชื่อถือ เหมือนที่ปรึกษาที่หวังดี ให้ความรู้เรื่องการวางแผนชีวิต/สุขภาพ/การเงิน ไม่ขายแข็ง ไม่ขู่ให้กลัว"
PERSONA = (
    f"แบรนด์: {BRAND}\n"
    f"กลุ่มลูกค้า: {AUDIENCE}\n"
    f"โทนการเขียน: {TONE}\n"
    "สำคัญ: ทุกคอนเทนต์ต้องเชื่อมโยงกับ 'การวางแผนประกันชีวิต/สุขภาพ/การเงิน หรือการดูแลความเสี่ยงในชีวิต' ให้เป็นธรรมชาติ "
    "ห้ามโยงไปเรื่องที่ไม่เกี่ยวกับประกัน/การเงิน/สุขภาพ เด็ดขาด และต้องไม่ให้คำแนะนำที่เป็นการการันตีผลตอบแทนเกินจริง"
)
# บล็อกข้อมูลติดต่อ — แทรกท้ายแคปชันทุกอัน (ก่อนแฮชแท็ก) — แก้เป็นข้อมูลตัวแทนของคุณ
CONTACT = (
    "📌 สนใจวางแผนประกันชีวิต/สุขภาพกับ AIA ปรึกษาฟรี ไม่มีค่าใช้จ่าย 📌\n"
    "ตัวแทน AIA: แซนด์ AIA\n"
    "Line ID: laseinesae\n"
    "Tel: 086-4239544"
)
...
WRITING_RULES = (
    ...
    "แฮชแท็กต้องเกี่ยวกับประกันชีวิต/ประกันสุขภาพ/วางแผนการเงิน/สุขภาพ/AIA ห้ามใช้แฮชแท็กนอกเรื่อง"
)
...
# โทนภาพ + กฎภาพ — แดง/ขาว, ภาพต้องสื่อถึงเนื้อหา, ห้ามมีโลโก้ AIA ในรูป
IMG_THEME = ("โทนสีหลักแดง-ขาว ดูสะอาด พรีเมียม ทันสมัย ตัวหนังสืออ่านง่าย เป็นโปสเตอร์โฆษณาประกัน. "
             ...
             "ห้ามมีโลโก้ AIA, ตราสัญลักษณ์, คำว่า AIA, สโลแกน หรือ wordmark แบรนด์ใดๆ ในรูปเด็ดขาด. "
             "ห้ามมีวงกลมรูปโปรไฟล์/อวตาร/รูปถ่ายในกรอบวงกลมในภาพ — แถบติดต่อให้เป็นข้อความล้วน (ชื่อ/Line/เบอร์) ไม่มีรูปหน้าในวงกลม. "
             "สำคัญ: ห้ามมีลายน้ำ (watermark) ใดๆ, ห้ามมีชื่อ/โลโก้เว็บสต็อกโฟโต้ (Getty Images, Shutterstock, iStock, Adobe Stock, Dreamstime, 123RF, Alamy ฯลฯ), "
             "ห้ามมี URL เว็บไซต์ ห้ามมีเครดิตภาพ/ลายเซ็นช่างภาพ — ต้องเป็นภาพถ่ายต้นฉบับที่สร้างขึ้นใหม่ทั้งหมด สะอาด ไม่มีตัวอักษรแปลกปลอมทับบนภาพ")
...
def _strip_polite(t):
    # ตัดคำลงท้ายสุภาพออก (ใช้กับข้อความในรูป — ในรูปไม่ต้องมี ค่ะ/ครับ)
    return re.sub(r"(นะคะ|นะค่ะ|ค่ะ|คะ|ครับผม|ครับ)", "", t or "").strip()
...
def _text_density_rule(level):
    ...
    try:
        ...
        level = 50
    if level <= 5:
        return ("สำคัญมาก: ในรูปนี้ 'ห้ามมีตัวหนังสือใดๆ ทั้งสิ้น' — ไม่มีพาดหัว ไม่มีคำบรรยาย ไม่มีแถบติดต่อ "
                "เป็นภาพถ่ายล้วนสะอาดสวยงามอย่างเดียว (ป้าย AIA จะถูกแปะภายหลัง ไม่ต้องวาดเอง)")
    if level <= 33:
        return ("ใส่ตัวหนังสือในรูป 'น้อยมาก' — มีแค่พาดหัวสั้นๆ วลีเดียวเด่นๆ ไม่เกิน 4-5 คำ "
                "ไม่ต้องมีข้อความรอง ไม่ต้องมีแถบติดต่อ เน้นพื้นที่ภาพเป็นหลัก ดูโปร่ง สะอาด")
    if level <= 66:
        return ("ใส่ตัวหนังสือในรูป 'ปริมาณปานกลาง' — พาดหัวเด่น 1 บรรทัด + ข้อความรองสั้นๆ 1 บรรทัด "
                "อ่านง่าย วางสมดุลกับภาพ ไม่รก (ใส่แถบติดต่อสั้นๆ ได้ถ้าพื้นที่พอ)")
    if level <= 90:
        return ("ใส่ตัวหนังสือในรูป 'ค่อนข้างเยอะ' แบบโปสเตอร์ข้อมูล — พาดหัว + ข้อความรอง + จุดเด่น (bullet) 2-3 ข้อ "
                "+ แถบติดต่อ จัดวางเป็นระเบียบอ่านง่าย")
...
def make_ai_images(hook, caption, n=2, mode="main", text_level=50):
    ...
    # ทุกกรณี: แปะตัวอักษร "AIA" มุมขวาบน · ในรูปไม่ต้องมีคำลงท้าย ค่ะ/ครับ
    hook, caption = _strip_polite(hook), _strip_polite(caption)
    ...
    if photo:
        prompt = ("สร้างโปสเตอร์โฆษณาประกันชีวิต/สุขภาพ 1:1 โดย 'คงใบหน้าของบุคคลในรูปต้นฉบับไว้ให้เหมือนเดิมเป๊ะที่สุด' — "
                  ...
                  "พื้นหลังออฟฟิศ/สตูดิโอสะอาด โทนแดง-ขาว ดูพรีเมียม. " + IMG_THEME + " " + density +
                  f"\nHook : {hook}\nCaption : {caption}")
        def _edit(**extra):
    ...
    prompt = ("สร้างรูปขนาด 1:1 ตามนี้ " + IMG_THEME +
              " ห้ามมีอวตาร/วงกลมรูปตัวแทนประกันในภาพ — "
              "ถ้ามีแถบติดต่อ ให้แสดงเฉพาะข้อความ (ชื่อตัวแทน/Line/เบอร์โทร) ไม่ต้องมีรูปถ่ายตัวแทน. "
              "สำคัญมาก: บุคคลทุกคนในภาพต้องเป็น 'คนสมมติที่ AI สร้างขึ้นใหม่ทั้งหมด' (fully synthetic, fictional) — "
              "ห้ามเหมือนหรืออ้างอิงใบหน้าของบุคคลจริง คนดัง ดารา นักการเมือง หรือนายแบบ/นางแบบสต็อกโฟโต้คนใดคนหนึ่งเด็ดขาด "
              "เป็นใบหน้าต้นฉบับที่ระบุตัวตนไม่ได้. ถ้าเป็นครอบครัว/ผู้สูงอายุ/เด็ก ก็ต้องเป็นบุคคลสมมติที่ AI สร้างขึ้นเองเช่นกัน. " + density +
              f"\nHook : {hook}\nCaption : {caption}")
...
_NEWS_RULE = (
    ...
    "ถ้าจะเชื่อมโยงกับการวางแผนประกัน/สุขภาพ/การเงิน ให้ทำเป็นมุมข้อคิด/บทเรียน/การเตือนให้เตรียมพร้อม ไม่ใช่กุข้อมูลหรือยัดขายแข็งๆ\n"
)
...
def gen_hooks(topic, news=None):
    ...
    if nb:
        sys = (
            ...
            f"กฎ: {WRITING_RULES}\n"
            "พาดหัวให้สั้น กระชับ สะดุดตา — ห้ามมีคำลงท้ายสุภาพ (ไม่ต้องมี ค่ะ/คะ/นะคะ/ครับ)\n"
...
def gen_captions(topic, hook, news=None):
    ...
    base = (
        ...
        f"กฎ: {WRITING_RULES}\n"
        "ใช้คำลงท้ายสุภาพแบบผู้หญิง (ค่ะ/คะ/นะคะ) ในแคปชัน ห้ามใช้ 'ครับ'\n"
...
def extract_url_text(url):
    # ดึงข้อความจากหน้าเว็บ (ตัดแท็ก HTML ออก)
    h = {"User-Agent": "Mozilla/5.0 (compatible; AIAContent/1.0)"}
    html = requests.get(url, headers=h, timeout=20).text
...
def summarize_doc(raw):
    # สรุปเอกสาร/หน้าเว็บประกัน → {title, summary} อิงเฉพาะข้อมูลในเอกสาร (ไว้ป้อนโหมดอิงข้อมูลจริง)
    raw = (raw or "").strip()
    if len(raw) < 30:
        return {"title": "", "summary": "", "ok": False, "error": "ดึงเนื้อหาไม่ได้/เนื้อหาน้อยเกินไป"}
    raw = raw[:7000]
    resp = client.chat.completions.create(
        model="gpt-5.4-mini",
        messages=[
            {"role": "system", "content":
                f"{PERSONA}\n"
                "นี่คือเนื้อหาจากเอกสาร/หน้าเว็บผลิตภัณฑ์ประกัน สรุปให้พร้อมเอาไปทำคอนเทนต์ "
                "ใช้เฉพาะข้อมูลที่อยู่ในเนื้อหา ห้ามแต่งตัวเลข/ความคุ้มครองเพิ่ม\n"
                "ตอบตามรูปแบบนี้เป๊ะ:\n"
                "หัวข้อ: <ชื่อผลิตภัณฑ์/ประเด็นสั้นๆ 1 บรรทัด>\n"
                "สรุป: <จุดเด่น/ความคุ้มครอง/กลุ่มเป้าหมาย 3-5 ข้อ ในย่อหน้าเดียว>"},
    ...
    for ln in out.splitlines():
        s = ln.strip()
        if s.startswith("หัวข้อ"):
            title = s.split(":", 1)[-1].strip()
        elif s.startswith("สรุป"):
            summary = s.split(":", 1)[-1].strip()
    if not summary:
        summary = out
    if not title:
        title = "ผลิตภัณฑ์ประกัน AIA"
    return {"title": title, "summary": summary, "ok": True}
...
def browse_news(query="", with_image=False):
    ...
    if query:
        ...
        focus = ("ช่วยค้นเว็บหาข่าว/ประเด็นที่น่าเอาไปทำคอนเทนต์ ประมาณ 20 หัวข้อ "
                 "แบ่งเป็นหลายแนว (เช่น ประกันสุขภาพ, ประกันชีวิต, โรคร้ายแรง/สุขภาพ, วางแผนการเงิน/ภาษี-ลดหย่อน, ข่าว AIA/วงการประกัน — ในและต่างประเทศ; "
